...
- **Never put `<br>` (or any block/disallowed tag) in an `inline_richtext` / `richtext` setting `default`.** This was the recurring "homepage shows the 404 page" bug across SOMNA, HIDE & HONEY/PRAIRIE and GLAZE. Shopify validates richtext setting **defaults** when the section loads; a `<br>` in an `inline_richtext` default makes that section **invalid**, and a JSON template that references the invalid section **fails to render → Shopify serves `404.json`** ("Lost the glow"). It's *homepage-only* because the offending section (a `hero` with a `"Line one<br>line two"` heading) lives only in `index.json`; the product page never references it, so the PDP keeps working — which masks the cause. **theme-check 3.26 and a liquidjs render do NOT catch this.** Fix: split the heading into two `text` settings and put the `<br>` in the *template markup* (`<h1>{{ line1 }}<br>{{ line2 }}</h1>`), not in a setting default. `inline_richtext` is fine for inline emphasis (`<em>`, `<strong>`, `<a>`) — it's `<br>`/block tags in the **default** that break it. After any build, grep `"default":.*<br` across `sections/` and `config/` and expect zero hits.
